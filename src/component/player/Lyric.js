...
class Lyric extends Component {
  ...
  setLyrObj(currentMusicLrc: {
    ...
    const lyrObj = []
    const lry = (currentMusicLrc && currentMusicLrc.lrc && currentMusicLrc.lrc.lyric) || this.props.currentMusicLrc.lrc.lyric
    const lryAry = lry.split('\n')   // 按照换行符切数组
    lryAry.forEach((val, index) => {
      const obj = {}   // 用于存放时间
      val = val.replace(/(^\s*)|(\s*$)/g, '')    // 正则,去除前后空格
      const indeofLastTime = val.indexOf(']')  // ]的下标
      const timeStr = val.substring(1, indeofLastTime) // 把时间切出来 0:04.19
      let minSec = ''
      const timeMsIndex = timeStr.indexOf('.')  // .的下标
      if (timeMsIndex !== -1) {
        // 存在毫秒 0:04.19
      ...
      const curTime = minSec.split(':')  // [0,04]
